...
class _LoginPageState extends State<LoginPage> {
  ...
  bool nullPassword = false;
  bool nullUsername = false;
  bool isChecked = false; // "Beni hatırla" checkbox durumu
  ...
  late SharedPreferences _prefs; // SharedPreferences instance'ı
  ...
  Future<void> _checkRememberedUser() async {
    final String? rememberToken = _prefs.getString('remember_me_token'); // SharedPreferences'tan oku
    if (rememberToken != null) {
      try {
        ...
        final response = await _dio.post(
          'http://192.168.14.143:8000/api/auto-login', // Otomatik giriş endpoint'i
          data: {
            'remember_token': rememberToken,
            'device_name': currentDeviceName, // Cihaz adını da gönder
          },
          options: Options(
        ...
        if (response.statusCode == 200) {
          ...
          final String? newRememberToken = response.data['remember_token']; // Yeni remember_token'ı al

          if (authToken != null) {
            await _prefs.setString('authToken', authToken); // API Token'ı kaydet
                      ...
                      child: CustomButton(text: "Giriş yap",snackText: "", snack: false,isNavigation: false,onPress: true,
                        ...
                        callback: (value) async {
                          ...
                          if (value == "ok") {
                            ...
                            try {
                              final response = await _dio.post( // Dio instance'ını kullan
                                ...
                                data: {
                                  'email': username,
                                  'password': password,
                                  "device_name": currentDeviceName, // Cihaz adını gönder
                                  "remember": isChecked, // "Beni hatırla" durumunu gönder
                                },
                                options: Options(
                                  followRedirects: true,
                                  validateStatus: (status) {
                              ...
                              if (response.statusCode == 200 ) {
                                final String? authToken = response.data['token']; // API Token
                                final String? rememberToken = response.data['remember_token']; // remember_token

                                if (authToken != null) {
                                  await _prefs.setString('authToken', authToken); // API Token'ı kaydet

                                  if (isChecked && rememberToken != null) {
                                    await _prefs.setString('remember_me_token', rememberToken); // remember_token'ı kaydet
                                  } else {
                                    await _prefs.remove('remember_me_token'); // Eğer 'beni hatırla' seçili değilse temizle
                                  }
