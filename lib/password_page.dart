...
import 'package:login_page_flutter/new_password_page.dart'; // Bu sayfa da güncellenecek
import 'package:login_page_flutter/widgets/custom_button.dart';
import 'package:login_page_flutter/widgets/custom_text_field.dart';
...
class _PasswordPageState extends State<PasswordPage> {
  ...
  void initState() {
    super.initState();
    // Bloc'un başlangıçta cooldown bilgisini yüklemesini tetikleyin
    context.read<PasswordResetBloc>().add(LoadPasswordResetCooldown());
  }
    ...
    return Scaffold(
      ...
      body: BlocListener<PasswordResetBloc, PasswordResetState>(
        listener: (context, state) {
          if (state is PasswordResetCodeSentSuccess) {
            showError(state.message);
            // Kod başarıyla gönderildiğinde NewPasswordPage'e geç
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => NewPasswordPage(email: state.email),
              ),
            );
          } else if (state is PasswordResetError) {
            showError(state.message);
          } else if (state is PasswordResetCooldownState) {
            // Cooldown aktifse, e-posta alanını önceden doldur ve salt okunur yap
            emailController.text = state.cooldownEmail;
          } else if (state is PasswordResetInitial) {
            // Cooldown bittiğinde email alanını temizle
        ...
        child: BlocBuilder<PasswordResetBloc, PasswordResetState>(
          builder: (context, state) {
            ...
            final bool isEmailReadOnly = state is PasswordResetCooldownState;
            ...
            // Butonun tıklanabilirliği için durum kontrolleri
            final bool canSendCode = !isLoading && isEmailValid && emailController.text.isNotEmpty;
            final bool canProceedToNewPasswordPage = isEmailReadOnly && emailController.text.isNotEmpty; // Cooldown varsa sadece ileri git
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: CustomButton(
                        ...
                        snackText: null,
                        snack: false,
                        isNavigation: false, // Navigasyonu butonda değil, BlocListener'da yöneteceğiz
                        onPress: (remainingSeconds > 0) ? canProceedToNewPasswordPage : canSendCode,
                        callback: (value) async {
                          if (value == "ok") {
                            final email = emailController.text.trim();
                            if (email.isEmpty || !isEmailValid) {
                            ...
                            if (remainingSeconds > 0) {
                              // Cooldown aktifse, sadece NewPasswordPage'e geç
                              if (email == cooldownEmail) { // Sadece aynı email için ileri git
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => NewPasswordPage(email: email),
                                ...
                                showError("Cooldown süresi boyunca başka bir e-posta ile işlem yapamazsınız.");
                              }
                            } else {
                              // Cooldown aktif değilse, yeni kod gönder
                              context.read<PasswordResetBloc>().add(SendPasswordResetCodeRequested(email: email));
                            }
                          }
                        },
                      ),
